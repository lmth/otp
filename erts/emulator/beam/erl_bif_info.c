    ...
    if (is_internal_pid(ses->target_pid)) {
        ...
        if (target) {
            ...
            if (target->chunked_backtrace == ses) {
                ...
                target->chunked_backtrace = NULL;                erts_resume(target, ERTS_PROC_LOCK_MAIN);
            }
...
backtrace_start_cb(Process *c_p, void *arg, int *redsp, ErlHeapFragment **bpp)
{
    ...
    if (available == 0) {
        ...
        if (tmp_ses.stepbuf)
            destroy_backtrace_dsbuf(tmp_ses.stepbuf);        erts_resume(c_p, ERTS_PROC_LOCK_MAIN);
        return am_done;
    }
...
backtrace_next_cb(Process *c_p, void *arg, int *redsp, ErlHeapFragment **bpp)
{
    ...
    if (available == 0) {
        ...
        erts_atomic_cmpxchg_nob(&ses->active, 0, 1);        if (ses->stepbuf)
            destroy_backtrace_dsbuf(ses->stepbuf);
        ses->stepbuf = NULL;
