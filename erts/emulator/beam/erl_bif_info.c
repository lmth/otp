...
{
    return erts_heap_factory_new_binary(hfact, (byte *) data, size, 0);
}
                      ...
                      int include_handle)
{
    ...
    Uint bin_hsz = (chunk_sz <= ERL_ONHEAP_BIN_LIMIT)
                       ? heap_bin_size(chunk_sz)
                       : PROC_BIN_SIZE;
...
cleanup_origin_cb(Process *c_p, void *arg, int *redsp, ErlHeapFragment **bpp)
{
    ...
    if (mon && mon->type == ERTS_MON_TYPE_SUSPEND) {
...
backtrace_session_monitor_cleanup(Process *c_p, ErtsBacktraceSession *ses)
{
    ...
    erts_monitor_list_delete(&ERTS_P_LT_MONITORS(c_p), &mdp->target);
    erts_atomic_read_band_acqb(&msp->state, ~ERTS_MSUSPEND_STATE_FLG_ACTIVE);
    ses->mon_mdp = NULL;

    erts_monitor_release(&mdp->target); /* refc: 2 → 1 */
...
backtrace_start_cb(Process *c_p, void *arg, int *redsp, ErlHeapFragment **bpp)
{
    ...
    if (ses->mon_mdp) {
        ...
        erts_monitor_list_insert(&ERTS_P_LT_MONITORS(c_p), &ses->mon_mdp->target);
        erts_atomic_read_bor_relb(&msp->state, ERTS_MSUSPEND_STATE_FLG_ACTIVE);
    }

    /* Self-suspend: stays suspended between _next calls. */
...
backtrace_next_cb(Process *c_p, void *arg, int *redsp, ErlHeapFragment **bpp)
{
    ...
    if (available == 0) {
        /* All data has been delivered — clean up monitor and resume target. */
        backtrace_session_monitor_cleanup(c_p, ses);
        if (ses->stepbuf)
            destroy_backtrace_dsbuf(ses->stepbuf);
        erts_free(ERTS_ALC_T_SIG_DATA, ses);
